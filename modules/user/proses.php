...
if (empty($_SESSION['username']) && empty($_SESSION['password'])) {
  ...
  if ($_GET['act'] == 'insert') {
    if (isset($_POST['Guardar'])) {
      ...
      $query = mysqli_query($mysqli, "INSERT INTO usuaerio(username, password, name_user, permisos_acceso)
  ...
  elseif ($_GET['act'] == 'update') {
    if (isset($_POST['Guardar'])) {
      if (isset($_POST['id_user'])) {
        ...
        $username = mysqli_real_escape_string($mysqli, trim($_POST['username']));
        $password = md5($mysqli, trim($_POST['password']));
        $name_user = mysqli_real_escape_string($mysqli, trim($_POST['name_user']));
  ...
  elseif ($_GET['act'] == 'on') {
    if (isset($_GET['id'])) {
      ...
      $query = mysqli_query($mysqli, "UPDATE usuario SET status = '$status' WHERE id_user = '$id_user'")
                                      or die('Error: '.mysqli_error($mysqli));
      if ($query) {
  ...
  elseif ($_GET['act'] == 'off') {
    if (isset($_GET['id'])) {
      ...
      $query = mysqli_query($mysqli, "UPDATE usuario SET status = '$status' WHERE id_user = '$id_user'")
                                      or die('Error: '.mysqli_error($mydqli));
      if ($query) {
