  ...
  <div class="row">
    <div class="col-lg-3 col-xs-6">
      ...
      <div class="small-box" style="background-color:#00c0ef;color:#fff">
        <div class="inner">
          ...
          $query = mysqli_query($mysqli, "SELECT COUNT(codigo) as numero FROM medicamentos") or die ('Error: '.mysqli_error($mysqli));
        ...
        if ($_SESSION['permisos_acceso']!='gerente') { ?>
          <a href="?module=form_medicines&form=add" class="small-box-footer" title="Agregar" data-toglle="tooltip"><i class="fa fa-plus"></i></a>
      <?php
      } else { ?>
    ...
    <div class="col-lg-3 col-xs-6">
        ...
        <div class="small-box" style="background-color:#00a65a;color:#fff">
          <div class="inner">
            <?php
            $query = mysqli_query($mysqli, "SELECT COUNT(codigo_transaccion) as numero FROM transaccion_medicamentos") or die ('Error '.mysqli_error($mysqli));
            $data = mysqli_fetch_assoc($query);
            ?>
            <h3><?php echo $data['numero']; ?></h3>
